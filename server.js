...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    db.run('INSERT INTO users (username, password, name, settings) VALUES (?, ?, ?, ?)', 
      [username, hashedPassword, name, JSON.stringify(defaultSettings)], function(err) {
        if (err) {
...
app.post('/api/chats', authenticateToken, (req, res) => {
  ...
  console.log('Создание чата:', { currentUserId, otherUserId });
  ...
  db.get('SELECT id, username, name, avatar, status FROM users WHERE id = ?', [otherUserId], (err, user) => {
    if (err) {
      ...
      [currentUserId, otherUserId, otherUserId, currentUserId], (err, existingChat) => {
        ...
        db.run(`INSERT INTO chats (user1_id, user2_id, last_message) VALUES (?, ?, ?)`, 
          [currentUserId, otherUserId, 'Чат создан'], function(err) {
            ...
            console.log('Новый чат создан с ID:', this.lastID);
            res.json({ 
              id: this.lastID, 
